...
def read_xlsx(path, table):
    ...
    frame = polars.read_excel(source=path, table_name=table, has_header=True)
    print(frame)
    return frame
...
def sp_emp():
    ...
    bk_sp_emp = return_query(
        FMCSQL.polars_conn, f'SELECT * FROM {cfg["sp_emp"]["bk_table"]}'
    )
    writecsv_from_frame(bk_sp_emp, cfg["sp_emp"]["bk_dest"])
    sp_emp = read_xlsx(cfg["sp_emp"]["src_filename"], cfg["sp_emp"]["src_table"])
    ...
    execute_sql(FMCSQL.pyodbc_conn, f'EXECUTE {cfg["sp_emp"]["u_script"]}')
    execute_sql(FMCSQL.pyodbc_conn, f'DROP TABLE {cfg["sp_emp"]["tgt_table"]}')
...
def sp_res():
    ...
    bk_sp_emp = return_query(
        FMCSQL.polars_conn, f'SELECT * FROM {cfg["sp_res"]["bk_table"]}'
    )
    writecsv_from_frame(bk_sp_emp, cfg["sp_res"]["bk_dest"])
    sp_res = read_xlsx(cfg["sp_res"]["src_filename"], cfg["sp_res"]["src_table"])
    ...
    execute_sql(FMCSQL.pyodbc_conn, f'EXECUTE {cfg["sp_res"]["u_script"]}')

    execute_sql(FMCSQL.pyodbc_conn, f'DROP TABLE {cfg["sp_res"]["tgt_table"]}')
...
def main():
    ...
    cfg = read_toml("sp_cfg.toml")
    FMCSQL = SQLServer(cfg["sql"])
    execute_sql(FMCSQL.pyodbc_conn, f"EXECUTE u_domo_staff_pulse")
